...
class WPEAI_users
{
  ...
  public function import_user($user)
  {
    ...
    $this->wpdb->query(
      $this->wpdb->prepare("INSERT INTO $this->user_table_name (%s) VALUES (%s)", $fields_str, $values)
    );
    ...
    if (!empty($usermeta)) {
      foreach ($usermeta as $meta) {
        $this->wpdb->query(
          $this->wpdb->prepare("INSERT INTO $this->usermeta_table_name (user_id,meta_key,meta_value) VALUES (%d,%s,%s)", $user_id, $usermeta['meta_key'], $usermeta['meta_value'])
        );
      }
    }
  }
